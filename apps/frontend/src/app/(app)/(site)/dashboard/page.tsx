...
/**
 * Followers tab — configure the standalone `autoDmFollowers` plug that sends
 * a welcome DM to every new X follower. Decoupled from posts: the plug is
 * upserted via `POST /integrations/:id/plugs` and toggled with
 * `PUT /integrations/plugs/:plugId/activate`.
 */
const FollowersPanel: FC<{ xIntegrationId?: string }> = ({
  xIntegrationId,
}) => {
  const t = useT();
  ...
  const existing = useMemo(() => {
    return (plugs || []).find(
      (p: any) => p.plugFunction === 'autoDmFollowers'
    );
  }, [plugs]);
  ...
  const save = useCallback(async () => {
    ...
    try {
      if (enabled) {
        // Upsert and (re)activate the plug.
        ...
        // Deactivate existing plug without deleting its baseline/snapshot
        // data so re-enabling later doesn't re-DM the same people.
  ...
  }, [enabled, message, xIntegrationId, existing, refetch, t, toast]);
...
export default function DashboardPage() {
  ...
  const [queueTab, setQueueTab] = useState<
    'Scheduled Tweets' | 'Published Tweets' | 'Drafts' | 'Followers'
  >('Scheduled Tweets');
  ...
  // Date range: past 10 days through next 6 days. The past window is wide
  // enough so the "Published Tweets" / "Sent" lists can show the last ~10
  // days of activity (the user explicitly asked for "past 10 days"), while
  // the +6 days future window still shows everything currently queued.
  ...
  // Group filtered posts by day, showing 10 days before today through 7 days
  // after — matches the fetch window so the Published Tweets view can scroll
  // back through the last ~10 days of activity.
  const days = useMemo(() => {
    ...
    return Array.from({ length: TOTAL }).map((_, i) => {
      const offset = i - PAST_DAYS; // -10..0..+7
      ...
      <div className="flex flex-1 min-h-0 flex-col lg:flex-row w-full">
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-4 sm:p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-newBorder custom-scrollbar">
          <header className="mb-6">
            ...
            {!loadingPosts && queueTab !== 'Followers' && filteredPosts.length === 0 && (
